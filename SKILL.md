...
每次建档、重排、修改出生信息、从材料建档，或生成深度解读前，都必须自动执行排盘复审。复审不是用户菜单入口，不要求用户懂盘。

复审分三层：

1. 本地脚本排盘：四系统事实只来自本地引擎。
2. AI 自动复审：检查出生信息完整度、地点精度、真太阳时、时辰边界、23:00 换日、节气/月界、上升/宫位/月宿边界、字段缺失和 `degraded`。
3. 材料交叉验证：如果 `materials/` 中有用户选择的 PDF/JPG/PNG，可提取其中的出生信息、经纬度、时区、星盘类型和关键标签，用来和本地排盘结果对照。

材料只能作为复审证据，不能覆盖本地排盘事实。PDF 或图片里的星体落座、Nakshatra、紫微命宫、上升、宫位、大限等，最多用于“与本地结果是否一致”的检查；若冲突，必须提示用户确认出生信息或材料来源，不得静默采用材料结果。
...
普通用户只看到简短自然提醒，例如“我已经帮你核过盘，这次时辰不贴边界，四套系统都能稳定使用”。debug 或审盘场景下，才展示详细复审条目。
...
- 所有排盘结果必须经过自动复审后再进入解读；复审发现风险时，相关判断必须降级。
